...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m codex_improvement",
    ...
    capture.add_argument(
        ...
        default=None,
        help=(
            "Held-out replay result as a JSON object, for example "
            '{"status":"passed","ref":"tests/held_out"}'
        ),
    )
    capture.add_argument("--rejected-reason", default="")
    capture.add_argument("--conflict", action="append", default=[])
    capture.add_argument(
        "--multi-objective-score",
        default=None,
        help='Multi-objective scores as a JSON object, for example {"safety":1.0}',
    )
    capture.add_argument("--human-verdict", default="pending")
    capture.add_argument(
        "--causal-evidence",
        action="append",
        default=[],
        help="Causal evidence as kind=ref or kind=ref#hash.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "capture":
        signal = capture_signal(
            ...
            causal_evidence=_parse_evidence_args(args.causal_evidence),
            held_out_replay_result=_parse_json_object_arg(
                args.held_out_replay_result,
                "--held-out-replay-result",
            ),
            rejected_reason=args.rejected_reason,
            conflict_set=tuple(args.conflict),
            multi_objective_score=_parse_json_object_arg(
                args.multi_objective_score,
                "--multi-objective-score",
            ),
