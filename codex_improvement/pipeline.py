...
class ImprovementSignal:
    ...
    causal_evidence: tuple[dict[str, Any], ...] = field(default_factory=tuple)
    held_out_replay_result: dict[str, Any] = field(
        default_factory=lambda: {"status": "not_run"}
    )
    rejected_reason: str = ""
    conflict_set: tuple[str, ...] = field(default_factory=tuple)
    multi_objective_score: dict[str, Any] = field(default_factory=dict)
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        data["conflict_set"] = list(self.conflict_set)
        return data
...
def capture_signal(
    *,
    ...
    held_out_replay_result: dict[str, Any] | None = None,
    rejected_reason: str = "",
    conflict_set: tuple[str, ...] = (),
    multi_objective_score: dict[str, Any] | None = None,
    human_verdict: str = "pending",
    ...
    return ImprovementSignal(
        ...
        causal_evidence=tuple(causal_evidence),
        held_out_replay_result=_result_or_default(held_out_replay_result),
        rejected_reason=rejected_reason,
        conflict_set=tuple(conflict_set),
        multi_objective_score=dict(multi_objective_score or {}),
    ...
    return [
        TriageDecision(
            ...
            causal_evidence=tuple(
                item for item in row.get("causal_evidence", []) if isinstance(item, dict)
            ),
            held_out_replay_result=_result_or_default(row.get("held_out_replay_result")),
            rejected_reason=str(row.get("rejected_reason", "")),
            conflict_set=tuple(str(item) for item in row.get("conflict_set", [])),
            multi_objective_score=dict(row.get("multi_objective_score", {})),
            human_verdict=str(row.get("human_verdict", row.get("human_decision", "pending"))),
        )
...
def validate_signal(signal: dict[str, Any]) -> list[str]:
    ...
    _validate_evidence_list(errors, prefix, signal, "evidence")
    if "causal_evidence" in signal:
        _validate_evidence_list(errors, prefix, signal, "causal_evidence")
    ...
    _validate_result_object(errors, prefix, signal, "held_out_replay_result")
    ...
    if "rejected_reason" in signal and not isinstance(signal["rejected_reason"], str):
        errors.append(f"{prefix}: rejected_reason must be a string")
    if "conflict_set" in signal and not isinstance(signal["conflict_set"], list):
        errors.append(f"{prefix}: conflict_set must be a list")
    elif "conflict_set" in signal and not all(
        isinstance(item, str) for item in signal["conflict_set"]
    ):
        errors.append(f"{prefix}: conflict_set items must be strings")
    if "multi_objective_score" in signal:
        _validate_multi_objective_score(errors, prefix, signal["multi_objective_score"])
    if "human_verdict" in signal and signal["human_verdict"] not in HUMAN_DECISIONS:
        errors.append(f"{prefix}: invalid human_verdict {signal['human_verdict']!r}")
    return errors
...
def _validate_evidence_list(
    ...
    if not isinstance(record[key], list):
        ...
        return
    for index, item in enumerate(record[key], start=1):
        if not isinstance(item, dict):
            errors.append(f"{prefix}: {key}[{index}] must be an object")
            continue
        if not item.get("kind") or not item.get("ref"):
            errors.append(f"{prefix}: {key}[{index}] requires kind and ref")


def _validate_multi_objective_score(
    errors: list[str],
    prefix: str,
    value: object,
) -> None:
    if not isinstance(value, dict):
        errors.append(f"{prefix}: multi_objective_score must be an object")
        return
    for key, score in value.items():
        if not isinstance(key, str):
            errors.append(f"{prefix}: multi_objective_score keys must be strings")
        if isinstance(score, bool) or not isinstance(score, int | float):
            errors.append(f"{prefix}: multi_objective_score.{key} must be numeric")
...
def validate_triage_record(record: dict[str, Any]) -> list[str]:
    ...
    _validate_result_object(errors, signal_id, record, "held_out_replay_result")
    ...
    if "causal_evidence" in record:
        _validate_evidence_list(errors, signal_id, record, "causal_evidence")
    if "rejected_reason" in record and not isinstance(record["rejected_reason"], str):
        errors.append(f"{signal_id}: rejected_reason must be a string")
    if "conflict_set" in record and not isinstance(record["conflict_set"], list):
        errors.append(f"{signal_id}: conflict_set must be a list")
    elif "conflict_set" in record and not all(
        isinstance(item, str) for item in record["conflict_set"]
    ):
        errors.append(f"{signal_id}: conflict_set items must be strings")
    if "multi_objective_score" in record:
        _validate_multi_objective_score(errors, signal_id, record["multi_objective_score"])
    if "human_verdict" in record and record["human_verdict"] not in HUMAN_DECISIONS:
        errors.append(f"{signal_id}: invalid human_verdict {record['human_verdict']!r}")
    return errors
    ...
    for decision in decisions:
        lines.extend(
            [
                ...
                f"- Causal evidence: `{_format_evidence_inline(decision.causal_evidence)}`",
                f"- Replay result: `{_format_result_inline(decision.replay_result)}`",
                f"- Held-out replay result: "
                f"`{_format_result_inline(decision.held_out_replay_result)}`",
                f"- Qualifier result: `{_format_result_inline(decision.qualifier_result)}`",
                f"- Human decision: `{decision.human_decision}`",
                f"- Human verdict: `{decision.human_verdict}`",
                f"- Rejected reason: `{decision.rejected_reason or 'none'}`",
                f"- Conflict set: `{', '.join(decision.conflict_set) or 'none'}`",
                f"- Multi-objective score: "
                f"`{_format_score_inline(decision.multi_objective_score)}`",
...
def _triage_signal(signal: dict[str, Any]) -> TriageDecision:
    ...
    return TriageDecision(
        ...
        gates=gates,
        causal_evidence=_causal_evidence(signal),
        held_out_replay_result=_held_out_replay_result(signal, disposition),
        rejected_reason=_rejected_reason(signal, disposition),
        conflict_set=_conflict_set(signal),
        multi_objective_score=_multi_objective_score(signal),
        human_verdict=_human_verdict(signal),
    )
...
def _human_review_required(
    ...
    *,
    ...
    if change_type in {
        ...
        return True
    if str(signal.get("project_scope", "")) == "codex_foundation" and change_type == "code_change":
        return True
    if str(signal.get("candidate_diff_ref", "")).strip():
        return True
    if str(signal.get("status", "")) == "candidate_generated":
        return True
    affected_text = " ".join(str(item) for item in signal.get("affected_artifacts", []))
    affected_normalized = affected_text.replace("\\", "/").casefold()
    protected_fragments = (
        "/.codex/foundation/",
        "/.codex/skills/",
        "/.codex/plugins/",
    ...
    if any(fragment in affected_normalized for fragment in protected_fragments):
        return True
    return category == "security_budget_blocker"
...
def _held_out_replay_result(signal: dict[str, Any], disposition: str) -> dict[str, Any]:
    explicit = _result_or_default(signal.get("held_out_replay_result"))
    if _result_status(explicit) != "not_run":
        return explicit
    if disposition == "rejected":
        return {
    ...
    return {
        "status": "not_run",
        "reason": "held-out replay has not been run for this candidate yet",
    }
...
def _human_verdict(signal: dict[str, Any]) -> str:
    verdict = str(signal.get("human_verdict", signal.get("human_decision", "pending")))
    if verdict in HUMAN_DECISIONS:
        return verdict
    return "pending"


def _causal_evidence(signal: dict[str, Any]) -> tuple[dict[str, Any], ...]:
    explicit = signal.get("causal_evidence")
...
def _rejected_reason(signal: dict[str, Any], disposition: str) -> str:
    explicit = _optional_text(signal.get("rejected_reason"))
    if explicit:
        return explicit
    if disposition == "rejected":
        return "triaged as no_change"
    return ""


def _conflict_set(signal: dict[str, Any]) -> tuple[str, ...]:
    value = signal.get("conflict_set")
    if not isinstance(value, list):
        return ()
    return tuple(str(item) for item in value if str(item).strip())


def _multi_objective_score(signal: dict[str, Any]) -> dict[str, Any]:
    value = signal.get("multi_objective_score")
...
def _format_evidence_inline(evidence: tuple[dict[str, Any], ...]) -> str:
    if not evidence:
        return "none"
    parts = []
    for item in evidence:
        kind = _optional_text(item.get("kind")) or "evidence"
        ref = _optional_text(item.get("ref")) or "unknown"
        parts.append(f"{kind}={ref}")
    return ", ".join(parts)


def _format_score_inline(score: dict[str, Any]) -> str:
    if not score:
        return "none"
    return ", ".join(f"{key}={score[key]}" for key in sorted(score))
...
def _candidate_report(decision: TriageDecision) -> str:
    ...
    held_out_replay_lines = _format_result_block(decision.held_out_replay_result)
    ...
    return (
        ...
        "## Causal Evidence\n\n"
        f"{_format_evidence_block(decision.causal_evidence)}\n\n"
        "## Rationale\n\n"
        f"{decision.rationale}\n\n"
        "## Replay\n\n"
        f"{replay_lines}\n\n"
        "## Held-Out Replay\n\n"
        f"{held_out_replay_lines}\n\n"
        "## Qualifier\n\n"
        f"{qualifier_lines}\n\n"
        "## Conflict Set\n\n"
        f"{_format_list_block(decision.conflict_set)}\n\n"
        "## Multi-Objective Score\n\n"
        f"{_format_score_block(decision.multi_objective_score)}\n\n"
        "## Validation Gates\n\n"
        ...
        f"- Verdict: `{decision.human_verdict}`\n"
        f"- Rejected reason: `{decision.rejected_reason or 'none'}`\n"
    )


def _format_evidence_block(evidence: tuple[dict[str, Any], ...]) -> str:
    if not evidence:
        return "- `none`"
    lines = []
    for item in evidence:
        kind = _optional_text(item.get("kind")) or "evidence"
        ref = _optional_text(item.get("ref")) or "unknown"
        hash_value = _optional_text(item.get("hash"))
        suffix = f" hash={hash_value}" if hash_value else ""
        lines.append(f"- `{kind}`: `{ref}`{suffix}")
    return "\n".join(lines)


def _format_list_block(items: tuple[str, ...]) -> str:
    if not items:
        return "- `none`"
...
def _format_score_block(score: dict[str, Any]) -> str:
    if not score:
        return "- `none`"
    return "\n".join(f"- `{key}`: `{score[key]}`" for key in sorted(score))
