...
try {
  ...
  Invoke-Checked -Name "foundation ci/cd surface validation" -Command { uv run python -c "from pathlib import Path; root = Path('.'); required = ['.github/dependabot.yml', '.github/workflows/foundation-ci.yml', '.github/workflows/foundation-codeql.yml', '.github/workflows/foundation-dependency-review.yml', '.github/workflows/trusted-pr-auto-merge.yml', 'scripts/package-foundation.ps1', 'scripts/foundation-rollback-plan.ps1', 'scripts/write-foundation-repo-manifest.py', 'pipeline/FOUNDATION_MANIFEST.json', 'FOUNDATION_REPO_MANIFEST.json']; missing = [p for p in required if not (root / p).exists()]; print('foundation ci/cd surfaces ok' if not missing else 'missing: ' + ', '.join(missing)); raise SystemExit(1 if missing else 0)" }
}
finally {
  Pop-Location
}
