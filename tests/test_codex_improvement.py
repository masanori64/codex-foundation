...
from codex_improvement.cli import main
from codex_improvement.pipeline import (
    capture_signal,
...
def test_source_adoption_improvement_fields_roundtrip_into_candidate_report(tmp_path) -> None:
    signal = capture_signal(
        signal_id="sig_source_adoption",
        created_at="2026-06-26T00:00:00+00:00",
        source_type="eval_failure",
        source_ref="chatgpt-pro:source-adoption",
        severity="high",
        project_scope="codex_foundation",
        symptom="Self-improvement candidate needs held-out replay before promotion",
        root_cause_hypothesis="candidate generation and validation were not separated",
        affected_artifacts=("C:/Users/maasa/.codex/foundation/codex_improvement/pipeline.py",),
        proposed_change_type="code_change",
        evidence=(
            {
                "kind": "control_artifact",
                "ref": "research_x_integration_plan_2026-06-26.md",
                "not_research_x_evidence": True,
            },
        ),
        privacy_level="project_private",
        fault_step="separate candidate generation from held-out validation",
        responsible_artifact="C:/Users/maasa/.codex/foundation/codex_improvement/pipeline.py",
        candidate_diff_ref=".codex/improvement/candidates/sig_source_adoption.diff",
        replay_result={"status": "passed", "ref": "tests/train/replay.jsonl"},
        held_out_replay_result={"status": "passed", "ref": "tests/held_out/replay.jsonl"},
        qualifier_result={"status": "passed", "ref": "tests/test_codex_improvement.py"},
        causal_evidence=(
            {
                "kind": "chatgpt-pro-artifact",
                "ref": "downloads/research_x_integration_plan_2026-06-26.md",
            },
        ),
        conflict_set=("sig_route_overlap", "sig_skill_bloat"),
        multi_objective_score={"safety": 1.0, "specificity": 0.9, "maintenance_cost": 0.2},
    ...
    assert validate_triage_record(decision.as_dict()) == []
    assert decision.human_review_required is True
    assert decision.causal_evidence[0]["kind"] == "chatgpt-pro-artifact"
    assert decision.held_out_replay_result["status"] == "passed"
    assert decision.conflict_set == ("sig_route_overlap", "sig_skill_bloat")
    assert decision.multi_objective_score["specificity"] == 0.9
    assert decision.human_verdict == "pending"

    report = format_triage_report([decision])
    assert "Held-out replay result: `status=passed, ref=tests/held_out/replay.jsonl`" in report
    assert "Multi-objective score: `maintenance_cost=0.2, safety=1.0, specificity=0.9`" in report

    paths = create_candidate_reports([decision], tmp_path / "candidates")
    text = paths[0].read_text(encoding="utf-8")
    assert "## Causal Evidence" in text
    assert "## Held-Out Replay" in text
    assert "## Conflict Set" in text
    assert "## Multi-Objective Score" in text
    assert "downloads/research_x_integration_plan_2026-06-26.md" in text
    assert validate_candidate_dir(tmp_path / "candidates") == []


def test_codex_foundation_code_changes_always_require_human_review() -> None:
    signal = capture_signal(
        signal_id="sig_foundation_code",
        created_at="2026-06-26T00:00:00+00:00",
        source_type="manual",
        source_ref="gpt-pro-feedback",
        severity="medium",
        project_scope="codex_foundation",
        symptom="Foundation code candidate changes proposal-only governance",
        proposed_change_type="code_change",
        affected_artifacts=("C:/Users/maasa/.codex/foundation/skill_audit.py",),
        candidate_diff_ref=".codex/improvement/candidates/sig_foundation_code.diff",
        evidence=(
            {
                "kind": "control_artifact",
                "ref": "gpt_feedback_response.md",
                "not_research_x_evidence": True,
            },
        ),
        privacy_level="project_private",
    ...
    assert decision.proposed_change_type == "code_change"
    assert decision.human_review_required is True
...
def test_invalid_source_adoption_improvement_fields_are_rejected() -> None:
    signal = capture_signal(
        signal_id="sig_bad_source_adoption",
        created_at="2026-06-26T00:00:00+00:00",
        source_type="manual",
        source_ref="manual",
        severity="medium",
        project_scope="codex_foundation",
        symptom="Invalid source-adoption metadata should not be accepted",
        proposed_change_type="code_change",
        evidence=({"kind": "note", "ref": "manual"},),
        privacy_level="project_private",
        causal_evidence=({"kind": "source"},),
        held_out_replay_result={"status": "maybe"},
        conflict_set=("sig_a",),
        multi_objective_score={"safety": "high"},
        human_verdict="auto_apply",
    )

    errors = validate_signal(signal.as_dict())

    assert "sig_bad_source_adoption: causal_evidence[1] requires kind and ref" in errors
    assert "sig_bad_source_adoption: invalid held_out_replay_result.status 'maybe'" in errors
    assert "sig_bad_source_adoption: multi_objective_score.safety must be numeric" in errors
    assert "sig_bad_source_adoption: invalid human_verdict 'auto_apply'" in errors
...
def test_cli_capture_triage_propose_validate_roundtrip(tmp_path, capsys) -> None:
    ...
    assert (
        main(
            [
                ...
                '{"status":"passed","ref":"tests/held_out.jsonl"}',
                "--causal-evidence",
                "trace=turn-1",
                "--conflict",
                "sig_previous",
                "--multi-objective-score",
                '{"safety":1.0,"maintenance_cost":0.2}',
                "--human-verdict",
                "pending",
                "--evidence",
                "transcript=turn-1",
    ...
    assert rows[0]["held_out_replay_result"]["status"] == "passed"
    assert rows[0]["causal_evidence"][0]["kind"] == "trace"
    assert rows[0]["conflict_set"] == ["sig_previous"]
    assert rows[0]["multi_objective_score"]["safety"] == 1.0
    assert rows[0]["human_verdict"] == "pending"
